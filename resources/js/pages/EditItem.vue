<template>
    <main class="l-main">
        ...
        <form class="c-form" @submit.prevent="editItem" v-show="!loading">
            ...
            <div
                class="c-form__group"
                :class="{
                    success: errors.image !== null && errors.image.length === 0,
                    error: errors.image !== null && errors.image.length !== 0,
                }"
            >
                <label for="image">商品画像<span>＊必須</span></label>
                <button
                    type="button"
                ...
                <button
                    type="button"
                    class="c-button js-remove-button active"
                    @click="clearFile"
                >
                    ファイルを解除
                </button>
            ...
            <div class="c-form__group edit">
                <input
                    class="c-button"
                    type="submit"
                    value="編集"
                    :class="{
                        soldout: item.sell_flg === 1,
                    }"
                />
                <input
                    class="c-button"
...
import { OK, CREATED, BAD_REQUEST, UNPROCESSABLE_ENTITY } from "../util";
...
export default {
    ...
    methods: {
        ...
        previewFileForDrop(e) {
            ...
            this.item.image = e.dataTransfer.files[0];
        ...
        async getItem() {
            ...
            this.preview = "/storage/items/" + response.data[0].image;
        },
        // 商品を編集するメソッド
        async editItem() {
            if (this.item.sell_flg === 1) {
                this.$store.commit("message/setContent", {
                    content: "購入済み商品は編集できません！",
                    timeout: 5000,
                });
            ...
            formData.append("imageChangeFlg", this.imageChangeFlg);
            const response = await axios.post("/api/items/edit", formData);
        ...
        async deleteItem() {
            ...
            const response = await axios.get(
                `/api/items/delete?item_id=${this.item.id}`
            );
